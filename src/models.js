...
const { Uint64LE, Int64LE } = int64Buffer

function getCharArray (buffer, offset, length) {
  const array = new Uint8Array(buffer, offset, length)
  let data = ''
  for (const x of array) {
    const code = String.fromCharCode(x)
    if (code !== '\u0000') {
      data = data + code
    }
  }
...
function Header (buffer) {
  ...
  let position = 0
  this.file_signature = getCharArray(buffer, 0, 4)
  position += 4
  ...
  this.project_id_guid_data = [
    ...
    getCharArray(buffer, position + 8, 8)
  ]
  position += 16
  this.version = {}
  ...
  position += 1
  this.system_identifier = getCharArray(buffer, position, 32)
  position += 32
  this.generating_software = getCharArray(buffer, position, 32)
  position += 32
  this.file_creation = {}
...
function VariableLengthRecordHeader (buffer) {
  ...
  if (String(this.user_id) === 'LASF_Projection') {
    if (String(this.record_id) === '34736') {
      // GeoDoubleParamsTag
...
GeoKey.prototype.extractKeys = function (records) {
  const dataView = new DataView(records['34735'].data)
  let position = 0
  ...
  for (let i = 0; i < this.wNumberOfKeys; i++) {
    ...
    if (key.wTIFFTagLocation === 34736) {
      ...
      key.value = records['34737'].ascii_data.substr(
        Number(key.wValue_Offset),
        Number(key.wValue_Offset) + Number(key.wCount) - 1
      )
    } else {
  ...
  if (this.hasKey(3072)) {
    const epsgProjectionCode = Number(this.getKey(3072))
    if (epsgProjectionCode < 32761) {
      this.epsg_projection_code = epsgProjectionCode
...
GeoKey.prototype.getProjValueForKey = function (key) {
  ...
  const code = geotiff.codes[key]
  if (
    geotiff.reference[code.type] &&
    typeof geotiff.reference[code.type][String(this.getKey(key).value)] !== 'undefined'
  ) {
...
GeoKey.prototype.mapGeotiff = function () {
  ...
  for (const key of Object.keys(geotiff.codes)) {
    const code = geotiff.codes[key]
    ...
    if (this.hasKey(key)) {
      ...
      if (Number(this.proj4_values[code.proj_key]) === 32767) {
  ...
  for (const key of Object.keys(geotiff.codes)) {
    ...
    if (code.type === 'ProjLinearUnits' && code.proj_key && this.proj4_values[code.proj_key]) {
      if (this.proj4_values['+to_meter']) {
        this.proj4_values[code.proj_key] =
          Number(this.proj4_values[code.proj_key]) * Number(this.proj4_values['+to_meter'])
...
GeoKey.prototype.computeProj4Args = function () {
  ...
  if (this.geotiff.GTModelTypeGeoKey === 'ModelTypeProjected') {
    for (const key of Object.keys(this.proj4_values)) {
...
function PointRecord (buffer, header, pointRecordOptions, projection) {
  const dataView = new DataView(buffer)
  this.raw = [
...
function ClassificationTable (buffer) {
  ...
  for (let i = 0; i < 255; i++) {
    position = i * 16
    const classNumber = dataView.getUint8(position)
    const description = getCharArray(buffer, position + 1, 15)
    this[String(classNumber)] = description
  }
}

// See https://github.com/LASzip/LASzip/blob/master/src/laszip.cpp
